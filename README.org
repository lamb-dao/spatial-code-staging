...
#+begin_src lisp
  ...
  #fill nodata
  gdal_fillnodata.py 2023-07-04_bgr.tif OUTPUT.tif -md 100 -si 11 -of GTiff

  usage: gdal_fillnodata.py [-h] [-q] [-md max_distance] [-si smoothing_iterations] [-o [name=value ...]] [-mask filename] [-b band] [-of gdal_format] [-co [name=value ...]] src_filename dst_filename
  ...
  positional arguments:
    src_filename          The source raster file used to identify target pixels. Only one band is used.
    dst_filename          The new file to create with the interpolated result. If not provided, the source band is updated in place.

  options:
    -h, --help            show this help message and exit
    -q, -quiet            The script runs in quiet mode. The progress monitor is suppressed and routine messages are not displayed.
    ...
    -o [name=value ...]   Specify a special argument to the algorithm.
    -mask filename        Use the first band of the specified file as a validity mask (zero is invalid, non-zero is valid).
    -b band, -band band   The band to operate on, defaults to 1.
    -of gdal_format       Select the output format. Use the short format name.
    -co [name=value ...]  Creation options for the destination dataset.
#+end_src
